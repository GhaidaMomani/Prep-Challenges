...
function fullName(arr) {
    return arr.map(i => `${i.firstName} ${i.lastName}`);
}
...
function gradesAvg(arr) {
    ...
    arr.map(i => i["avg"] = average(i.gradsList) );
    return arr;
...
function studentsResult(arr) {
    let resultArray = gradesAvg(arr);
    resultArray .map(i => (i.avg >= 50.0) ? i["result"] = "Passed" : i["result"] = "Failed" );
    return resultArray ;
}
